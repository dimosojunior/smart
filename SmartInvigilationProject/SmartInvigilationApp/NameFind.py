...
face = cv2.CascadeClassifier(BASE_DIR+'/cascades/data/haarcascade_frontalface_default.xml')
glass_cas = cv2.CascadeClassifier(BASE_DIR+'/cascades/data/haarcascade_eye_tree_eyeglasses.xml')

WHITE = [255, 255, 255]

def FileRead():
    Info = open(BASE_DIR+"/Names.txt", "r")                       # Open th text file in readmode
...
def ID2Name(ID, conf):
    if ID > 0:
        ...
        NameString = "Name: " + Names[ID-1]                                 # Find the Name using the index of the ID
    else:
...
def AddName():
    ...
    Info = open(BASE_DIR+"/Names.txt", "r+")
